...
export const App: React.FC = () => {
  ...
  // 4. Messaging Listener - FIXED & SECURE
  useEffect(() => {
    if (!authUid) {
        setMessagesMap({});
        return;
    ...
    // שאילתה זו חייבת להתאים אחד-לאחד לחוק ב-Firebase:
    ...
    const unsubMessages = db.collection("messages")
        ...
        .onSnapshot(s => {
            ...
            console.error("Chat Listener Error (Check Rules!):", e);
        });
      ...
      <MessagingModal 
        ...
        onSendMessage={(rid, rn, s, c) => {
            ...
            const msg: Message = { 
              ...
              senderId: authUid, 
              receiverId: rid, 
              // IMPORTANT: This field must match the Security Rule query requirement
            ...
            db.collection("messages").add(msg).catch(e => {
                ...
                alert("שגיאה בשליחת הודעה. וודא שעדכנת את חוקי האבטחה ב-Firebase Console.");
            });
        }} 
