            ...
            {step === 1 ? (
                <div className="space-y-4">
                    ...
                    <div className="relative">
                        <textarea className="w-full bg-white border border-slate-300 rounded-xl p-4 pb-10 text-slate-900 placeholder-slate-400 text-sm focus:ring-2 focus:ring-brand-200 focus:border-brand-500 outline-none transition-all min-h-[140px] shadow-sm" placeholder='דוגמה: "אני בונה אתרים ומחפש ייעוץ משפטי עד סוף החודש בתל אביב..."' value={roughText + (isRecording && interimText ? ' ' + interimText : '')} onChange={(e) => { setRoughText(e.target.value); setInterimText(''); }}></textarea>
                        <button type="button" onClick={toggleRecording} className={`absolute bottom-3 left-3 p-2 rounded-full transition-all shadow-sm flex items-center justify-center z-10 ${isRecording ? 'bg-brand-500 text-white animate-pulse' : 'bg-slate-100 text-slate-500 hover:text-brand-600 hover:bg-brand-50 border border-slate-200'}`} title={isRecording ? 'עצור הקלטה' : 'הקלט הצעה קולית'}>{isRecording ? <Square className="w-4 h-4 fill-current" /> : <Mic className="w-4 h-4" />}</button>
                        {isRecording && <span className="absolute bottom-4 left-14 text-xs text-brand-600 font-medium animate-pulse flex items-center gap-1"><span className="w-2 h-2 bg-brand-500 rounded-full"></span>מקליט...</span>}
                    </div>
                    <button onClick={handleAiAssist} disabled={(!roughText && !interimText) || isAiLoading} className="w-full bg-brand-600 text-white border-2 border-brand-700 rounded-xl py-3.5 flex items-center justify-center gap-2 hover:bg-brand-700 disabled:opacity-50 transition-all font-bold text-base shadow-sm hover:shadow-md active:scale-95">{isAiLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5" />}<span>נסח ומלא פרטים אוטומטית</span></button>
                    <div className="relative flex py-2 items-center"><div className="flex-grow border-t border-slate-200"></div><span className="flex-shrink-0 mx-4 text-slate-400 text-xs font-medium">או</span><div className="flex-grow border-t border-slate-200"></div></div>
                    <button onClick={() => setStep(2)} className="w-full bg-white text-slate-700 border border-slate-300 rounded-xl py-3 text-sm font-bold hover:bg-slate-50 hover:text-slate-900 transition-colors shadow-sm">דלג ומלא לבד</button>
                </div>
            ) : (
                <form className="space-y-4">
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div onClick={() => setFormData({...formData, durationType: 'one-time'})} className={`cursor-pointer rounded-lg p-3 border transition-all ${formData.durationType === 'one-time' ? 'bg-white border-brand-500 shadow-sm ring-1 ring-brand-500' : 'bg-white border-slate-200 hover:border-slate-300'}`}><div className="flex items-center gap-2 mb-1"><Clock className={`w-4 h-4 ${formData.durationType === 'one-time' ? 'text-brand-600' : 'text-slate-500'}`} /><span className={`font-bold text-sm ${formData.durationType === 'one-time' ? 'text-brand-700' : 'text-slate-700'}`}>חד פעמי</span></div><p className="text-xs text-slate-500 leading-snug">פרויקט מוגדר בזמן (למשל: בניית אתר)</p></div>
                            <div onClick={() => setFormData({...formData, durationType: 'ongoing', expirationDate: ''})} className={`cursor-pointer rounded-lg p-3 border transition-all ${formData.durationType === 'ongoing' ? 'bg-white border-blue-500 shadow-sm ring-1 ring-blue-500' : 'bg-white border-slate-200 hover:border-slate-300'}`}><div className="flex items-center gap-2 mb-1"><Repeat className={`w-4 h-4 ${formData.durationType === 'ongoing' ? 'text-blue-600' : 'text-slate-500'}`} /><span className={`font-bold text-sm ${formData.durationType === 'ongoing' ? 'text-blue-700' : 'text-slate-700'}`}>מתמשך</span></div><p className="text-xs text-slate-500 leading-snug">ריטיינר או שירות קבוע לאורך זמן</p></div>
                        </div>
                    </div>
                    {formData.durationType === 'one-time' && (
                        <div className="animate-in fade-in slide-in-from-top-1 duration-300"><label className="block text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />תאריך יעד לביצוע (אופציונלי)</label><input type="date" className={inputClassName} value={formData.expirationDate} onChange={(e) => setFormData({...formData, expirationDate: e.target.value})} min={new Date().toISOString().split('T')[0]} /><p className="text-[10px] text-slate-500 mt-1 mr-1">המודעה תוסר אוטומטית מהלוח לאחר תאריך זה.</p></div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-slate-700 mb-1.5">מה אני נותן</label><input type="text" className={inputClassName} value={formData.offeredService} onChange={(e) => setFormData({...formData, offeredService: e.target.value})} /></div><div><label className="block text-xs font-bold text-slate-700 mb-1.5">מה אני מחפש</label><input type="text" className={inputClassName} value={formData.requestedService} onChange={(e) => setFormData({...formData, requestedService: e.target.value})} /></div></div>
                    <div><label className="block text-xs font-bold text-slate-700 mb-1.5">תגיות (מופרד בפסיקים)</label><div className="relative"><Tag className="w-4 h-4 absolute right-3 top-3 text-slate-400" /><input type="text" className={`${inputClassName} pr-10`} placeholder="לדוגמה: דיגיטל, ספורט, עיצוב" value={formData.tags} onChange={(e) => setFormData({...formData, tags: e.target.value})} /></div></div>
