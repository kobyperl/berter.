...
export const CreateOfferModal: React.FC<CreateOfferModalProps> = ({ 
  ...
  const handleAiAssist = async () => {
    ...
    if (result) {
        setFormData(prev => ({ ...prev, title: result.title, description: result.description, offeredService: result.offeredService, requestedService: result.requestedService, location: result.location || 'כל הארץ', tags: result.tags.join(', '), durationType: result.durationType === 'ongoing' ? 'ongoing' : 'one-time', expirationDate: result.expirationDate || '' }));
        setStep(2);
    } else {
            ...
            <div className="p-5 sm:p-8 bg-white overflow-y-auto custom-scrollbar flex-1">
                {step === 1 ? (
                    <div className="space-y-6">
                        ...
                        <button onClick={handleAiAssist} disabled={(!roughText && !interimText) || isAiLoading} className="w-full bg-brand-600 text-white rounded-xl py-3.5 flex items-center justify-center gap-2 hover:bg-brand-700 disabled:opacity-50 font-bold shadow-md transition-all active:scale-95">{isAiLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5" />} ייעל הצעה בעזרת AI</button>
                        ...
                        <button onClick={() => setStep(2)} className="w-full bg-white text-slate-700 border border-slate-200 rounded-xl py-3 text-sm font-bold shadow-sm hover:bg-white transition-colors">דלג ומלא לבד</button>
                    </div>
                ) : (
