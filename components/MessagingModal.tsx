                ...
                {activeConversationId ? (
                    <>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                            {activeMessages.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-slate-500 text-center bg-white p-6 rounded-xl mx-auto max-w-sm mt-10 shadow-sm">
                                    ...
                                    {initialSubject && (
                                        <div className="mt-2 text-sm bg-slate-50 p-2 rounded text-slate-600 border border-slate-200">
                                            נושא: {initialSubject}
                                        </div>
                                    )}
                                </div>
                                        ...
                                        <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                            {idx === 0 && msg.subject !== "Chat" && (
                                                 <div className="w-full text-center my-2">
                                                     <span className="bg-slate-200 text-slate-600 text-[10px] px-2 py-1 rounded-full shadow-sm border border-slate-300">
                                                        נושא: {msg.subject}
                                                     </span>
                                                 </div>
                                            )}
                                            ...
                                            <div 
                                                className={`max-w-[85%] sm:max-w-[70%] rounded-2xl px-4 py-2 shadow-sm relative text-sm ${
                                                    isMe 
                                                    ? 'bg-brand-500 text-white rounded-tl-2xl rounded-tr-none' 
                                                    : 'bg-white text-slate-900 rounded-tl-none rounded-tr-2xl border border-slate-100'
                                                }`}
                                            >
                                                <p className="whitespace-pre-wrap leading-relaxed">{msg.content}</p>
                                                <div className={`text-[10px] mt-1 flex items-center gap-1 ${isMe ? 'justify-end text-brand-100' : 'text-slate-400'}`}>
                                                    {formatBubbleDate(msg.timestamp)}
                                                    {isMe && (
                                                        <span>{msg.isRead ? <span className="text-brand-200">✓✓</span> : <span>✓</span>}</span>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div className="bg-white p-3 flex items-center gap-2 border-t border-slate-200 shrink-0">
                            <input 
                                type="text"
                                className="flex-1 bg-slate-50 border border-slate-300 text-slate-900 rounded-full py-3 px-5 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all placeholder-slate-400 shadow-inner"
