                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                         ...
                         <div className="w-full sm:w-auto flex justify-end">
                            <button 
                                ...
                                disabled={isExpired}
                                className={`text-white text-xs font-bold px-4 py-2 rounded-lg transition-colors flex items-center gap-2 ${isExpired ? 'bg-slate-300 cursor-not-allowed' : 'bg-slate-900 hover:bg-slate-800'}`}
                            >
      ...
      <div className="bg-slate-50 px-5 py-3 border-t border-slate-100 flex items-center justify-between gap-4">
         ...
         <div className="flex flex-col sm:flex-row gap-1 sm:gap-3 text-xs text-slate-400">
            <div className="flex items-center gap-1">
                ...
                <span className="leading-none">{offer.location}</span>
            </div>
            <div className="flex items-center gap-1">
                <Calendar className="w-3 h-3 flex-shrink-0" />
                <span className="leading-none">{new Date(offer.createdAt).toLocaleDateString('he-IL')}</span>
            </div>
         </div>
         ...
         <button 
            ...
            disabled={isExpired}
            className={`text-white text-sm font-bold px-4 py-2 rounded-lg transition-colors flex items-center gap-2 shrink-0 ${isExpired ? 'bg-slate-300 cursor-not-allowed' : 'bg-slate-900 hover:bg-slate-800'}`}
         >
