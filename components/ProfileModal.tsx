...
export const ProfileModal: React.FC<ProfileModalProps> = ({ 
  isOpen, onClose, profile, currentUser, userOffers, onDeleteOffer, onEditOffer, onUpdateProfile, onContact, onRate, availableCategories, availableInterests, onApproveUpdate, onRejectUpdate, startInEditMode = false
}) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-2 sm:p-4" role="dialog" aria-modal="true">
      ...
      <div className="bg-white rounded-2xl text-right overflow-hidden shadow-xl transform transition-all w-full max-w-4xl relative z-10 max-h-[92vh] flex flex-col">
            {/* Fixed Navigation Bar inside modal */}
            <div className="absolute top-0 left-0 right-0 p-3 flex justify-between items-center z-20 pointer-events-none">
                <button onClick={onClose} className="bg-black/20 hover:bg-black/30 text-white p-2 rounded-full backdrop-blur-md pointer-events-auto transition-colors"><X className="w-5 h-5" /></button>
                {(isOwnProfile || isAdmin) && !isEditing && (
                    <button onClick={() => setIsEditing(true)} className="bg-white/90 text-brand-700 px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm flex items-center gap-2 pointer-events-auto transition-all hover:bg-white active:scale-95"><Pencil className="w-3 h-3" />עריכה</button>
                )}
            </div>

            {/* Main Scrollable Content */}
            <div className="overflow-y-auto custom-scrollbar flex-1">
                {/* Header Background (Now inside the scrollable container) */}
                <div className="h-28 sm:h-36 bg-gradient-to-r from-brand-500 to-teal-600 relative shrink-0"></div>

                <div className="px-4 sm:px-8 pb-8">
                    {isEditing && editFormData ? (
                        <form onSubmit={handleSave} className="-mt-14 sm:-mt-16 relative bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-slate-200 space-y-5 z-10">
                            <div className="flex justify-center">
                                <div className="relative group cursor-pointer" onClick={() => avatarInputRef.current?.click()}>
                                    <img src={editFormData.avatarUrl} className="w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-white shadow-md bg-white object-cover aspect-square" />
                                    <div className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><Camera className="w-6 h-6 text-white" /></div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div><label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">לינק לתיק עבודות</label><input className={inputClassName} value={editFormData.portfolioUrl} onChange={e => setEditFormData(prev => prev ? ({...prev, portfolioUrl: e.target.value}) : null)} /></div>
                                <div><label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">תיאור הלינק (עד 15 תווים)</label><input maxLength={15} className={inputClassName} value={editFormData.portfolioLinkText || ''} onChange={e => setEditFormData(prev => prev ? ({...prev, portfolioLinkText: e.target.value}) : null)} /></div>
                            </div>
                        ...
                        <>
                            <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-end -mt-14 sm:-mt-16 mb-6 gap-4 z-10">
                                <div className="flex items-end gap-4">
                                    <img src={displayProfile.avatarUrl} className="w-28 h-28 sm:w-36 sm:h-36 rounded-full border-4 border-white shadow-xl bg-white object-cover aspect-square" />
                                ...
                                {displayProfile.portfolioUrl && (
                                    <a 
                                        ...
                                        target="_blank" 
                                        rel="noreferrer" 
                                        className="bg-white border border-slate-200 text-slate-700 px-5 py-2 rounded-xl text-xs font-bold hover:text-brand-600 shadow-sm transition-all w-full sm:w-auto text-center flex items-center justify-center gap-2"
                                    >
                                        <ExternalLink className="w-4 h-4" />
                                        {displayProfile.portfolioLinkText || cleanUrlForDisplay(displayProfile.portfolioUrl)}
                                    </a>
                                )}
                            </div>
                            ...
                            <div className="border-t pt-6">
                                <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2 text-sm uppercase tracking-wider text-slate-400">הצעות שפורסמו <span className="bg-white border border-slate-200 text-slate-600 text-[10px] px-2 py-0.5 rounded-full">{userOffers.length}</span></h3>
                                <div className="space-y-3 pr-1">
                                    {userOffers.length > 0 ? userOffers.map(offer => (<OfferCard key={offer.id} offer={offer} viewMode="compact" currentUserId={currentUser?.id} onUserClick={() => {}} onContact={() => { onClose(); onContact(displayProfile!); }} onRate={onRate} onDelete={isOwnProfile ? onDeleteOffer : undefined} onEdit={isOwnProfile ? onEditOffer : undefined} />)) : (<p className="text-slate-400 text-center py-8 bg-white rounded-xl border-dashed border">אין הצעות פעילות.</p>)}
                                </div>
                            </div>
                        </>
                    )}
