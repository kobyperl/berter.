      ...
      return (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-900 bg-opacity-75">
            <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center animate-in fade-in zoom-in shadow-2xl">
                ...
                <p className="text-slate-600 mb-6 leading-relaxed">העדכון הועבר לבדיקת מנהל המערכת ויופיע באתר לכולם מיד לאחר האישור.</p>
                    ...
                    <form onSubmit={handleSave} className="-mt-12 relative bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                         <div className="mb-4 flex justify-center">
                             <div className="text-center relative group cursor-pointer" onClick={() => avatarInputRef.current?.click()}>
                                <img src={editFormData.avatarUrl} alt="Preview" className="w-24 h-24 rounded-full border-4 border-white shadow-md bg-white mx-auto mb-2 object-cover aspect-square group-hover:opacity-75 transition-opacity" />
                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                    <div className="bg-black bg-opacity-50 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><Camera className="w-5 h-5" /></div>
                                </div>
                                <span className="text-xs text-brand-600 font-bold hover:underline">{isUploading ? 'מעלה...' : 'שנה תמונה'}</span>
                         ...
                         <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
                             ...
                             <div className="flex gap-2 mb-3">
                                <div className="relative flex-1">
                                    ...
                                    <input type="text" list="profile-interests-list" className={`${inputClassName} pr-9`} placeholder="בחר או הוסף" value={interestInput} onChange={e => setInterestInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && (e.preventDefault(), handleAddInterest())} />
                                    <datalist id="profile-interests-list">{availableInterests.map(int => <option key={int} value={int} />)}</datalist>
                                </div>
                                <button type="button" onClick={handleAddInterest} disabled={!interestInput.trim()} className="bg-brand-600 text-white px-4 rounded-xl hover:bg-brand-700 disabled:opacity-50 transition-colors"><Plus className="w-5 h-5" /></button>
                             </div>
                             <div className="flex flex-wrap gap-2 min-h-[40px] items-center">
                                 {editFormData.interests?.map((interest, idx) => (
                                     <span key={idx} className="bg-white border border-slate-200 text-slate-700 px-3 py-1.5 rounded-full text-sm flex items-center gap-1 shadow-sm group hover:border-red-200">{interest}<button type="button" onClick={() => handleRemoveInterest(interest)} className="text-slate-400 hover:text-red-500 transition-colors"><X className="w-3.5 h-3.5" /></button></span>
                                 ))}
                             </div>
                         </div>
                         <div className="mb-6 border-t border-slate-100 pt-4">
                            <label className="block text-xs font-bold text-slate-700 mb-3">תמונות לגלריה</label>
                            <div className="flex gap-2 mb-3 items-center">
                                <button type="button" onClick={() => !isUploading && portfolioInputRef.current?.click()} disabled={isUploading} className="bg-slate-100 text-slate-700 px-3 py-2.5 rounded-lg text-sm font-bold hover:bg-slate-200 flex items-center gap-2 border border-slate-200 disabled:opacity-50 transition-all">{isUploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />}{isUploading ? 'מעלה...' : 'העלאת תמונות'}</button>
                                ...
                                <input type="text" className={inputClassName} placeholder="או הדבק קישור..." value={newImageUrl} onChange={e => setNewImageUrl(e.target.value)} />
                                <button type="button" onClick={handleAddImage} disabled={!newImageUrl} className="bg-brand-50 text-brand-600 px-3.5 py-2.5 rounded-lg hover:bg-brand-100 border border-brand-100"><Plus className="w-5 h-5" /></button>
                            </div>
                            <div className="flex flex-wrap gap-3 mt-3">
                                {editFormData.portfolioImages?.map((img, idx) => (
                                    <div key={idx} className="relative w-20 h-20 rounded-lg overflow-hidden group border border-slate-200 shadow-sm"><img src={img} alt="" className="w-full h-full object-cover" /><button type="button" onClick={() => handleRemoveImage(idx)} className="absolute inset-0 bg-black bg-opacity-50 text-white opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity"><X className="w-5 h-5" /></button></div>
                                ))}
                            </div>
                         </div>
                         <div className="flex gap-3 justify-end pt-2 border-t border-slate-100">
                             <button type="button" onClick={() => setIsEditing(false)} className="px-5 py-2.5 text-slate-600 text-sm font-medium hover:bg-slate-50 rounded-lg">ביטול</button>
                             <button type="submit" disabled={isUploading || isSaving} className="bg-brand-600 hover:bg-brand-700 text-white px-6 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 shadow-sm transition-all disabled:opacity-50">{isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}{isAdmin ? 'שמור ועדכן מידית' : 'שמור (שלח לאישור)'}</button>
                         </div>
                    </form>
                ) : (
                    <>
                        <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-end -mt-12 mb-8 gap-4">
                            ...
                            {displayProfile.portfolioUrl && (
                                <div className="w-full sm:w-auto flex justify-start sm:justify-end sm:mb-2">
                                    <a href={displayProfile.portfolioUrl} target="_blank" rel="noreferrer" className="bg-white border border-slate-200 text-slate-700 px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-slate-50 hover:text-brand-600 flex items-center justify-center gap-2 shadow-sm transition-all w-full sm:w-auto"><ExternalLink className="w-4 h-4" />תיק עבודות</a>
                                </div>
                            )}
                        </div>
                        {displayProfile.bio && (
                            <div className="mb-8"><h3 className="font-bold text-slate-900 mb-3 text-lg">אודות</h3><p className="text-slate-600 leading-relaxed bg-slate-50 p-4 rounded-xl border border-slate-100">{displayProfile.bio}</p></div>
                        )}
                             ...
                             <div className="mt-8 border-t-2 border-yellow-200 pt-6">
                                {isAdmin ? (
                                    <div className="bg-yellow-50 rounded-xl border border-yellow-200 p-4">
                                        <div className="flex items-center gap-3 mb-4"><div className="bg-yellow-100 p-2 rounded-full text-yellow-600"><AlertCircle className="w-6 h-6" /></div><div><h3 className="font-bold text-yellow-900 text-lg">בקשה לשינוי פרופיל</h3><p className="text-sm text-yellow-700">אתה צופה בגרסה המעודכנת. אשר או דחה את השינויים.</p></div></div>
                                        <div className="flex gap-3 justify-end"><button onClick={() => onRejectUpdate && onRejectUpdate(profile.id)} className="flex items-center gap-2 bg-white border border-red-200 text-red-600 px-6 py-2.5 rounded-lg text-sm font-bold hover:bg-red-50"><XCircle className="w-4 h-4" />דחה</button><button onClick={() => onApproveUpdate && onApproveUpdate(profile.id)} className="flex items-center gap-2 bg-brand-600 text-white px-6 py-2.5 rounded-lg text-sm font-bold hover:bg-brand-700 shadow-sm"><CheckCircle className="w-4 h-4" />אשר שינויים</button></div>
                                    </div>
                                ) : (
                                    <div className="bg-yellow-50 rounded-xl border border-yellow-200 p-4 flex flex-col sm:flex-row items-center gap-4 text-center sm:text-right">
                                        <div className="bg-yellow-100 p-3 rounded-full text-yellow-600 shrink-0"><Loader2 className="w-6 h-6 animate-spin" /></div>
                                        <div><h3 className="font-bold text-yellow-900 text-lg">השינויים נשמרו וממתינים לאישור</h3><p className="text-sm text-yellow-700 mt-1">אתה צופה בתצוגה מקדימה של הפרופיל. שאר המשתמשים יראו את העדכון רק לאחר אישור מנהל המערכת.</p></div>
                                    </div>
                                )}
                             </div>
                         )}
      ...
      {lightboxIndex !== null && displayProfile.portfolioImages && (
          <div className="fixed inset-0 z-[100] bg-slate-900/95 flex items-center justify-center p-4 animate-in fade-in duration-300">
              <button 
                ...
                className="absolute top-6 left-6 text-white/70 hover:text-white p-2 transition-colors z-50 bg-black/20 rounded-full"
              >
                  <X className="w-8 h-8" />
              </button>
              ...
              <div className="relative w-full max-w-5xl flex items-center justify-center group">
                  {displayProfile.portfolioImages.length > 1 && (
                      <>
                          <button 
                            ...
                            className="absolute right-0 sm:-right-16 text-white/50 hover:text-white p-4 transition-all hover:scale-110 z-50"
                          >
                              <ChevronRight className="w-12 h-12" />
                          </button>
                          <button 
                            onClick={handlePrevImage}
                            className="absolute left-0 sm:-left-16 text-white/50 hover:text-white p-4 transition-all hover:scale-110 z-50"
                          >
                  ...
                  <div className="relative overflow-hidden rounded-lg shadow-2xl max-h-[90vh]">
                      <img 
                        src={displayProfile.portfolioImages[lightboxIndex]} 
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/60 to-transparent text-white text-center">
                          <span className="text-sm font-bold opacity-80">
                            {lightboxIndex + 1} / {displayProfile.portfolioImages.length}
                          </span>
                      </div>
                  </div>
              </div>
